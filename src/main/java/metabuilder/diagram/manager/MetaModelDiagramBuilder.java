...
import metabuilder.diagram.manager.constraints.ConstraintSet;
import metabuilder.diagram.manager.constraints.EdgeConstraints;
import metabuilder.diagram.manager.constraints.MetaModelDiagramEdgeConstraints;
import metabuilder.diagram.nodes.AbstractPackageNode;
import metabuilder.diagram.nodes.PackageNode;
import metabuilder.diagram.nodes.TypeNode;
import metabuilder.geom.Point;
import metabuilder.viewers.nodes.NodeViewerRegistry;

/**
* A builder for metamodel diagrams.
*/
public class MetaModelDiagramBuilder extends DiagramBuilder
{
	...
	private static final ConstraintSet CONSTRAINTS = new ConstraintSet(
			...
			MetaModelDiagramEdgeConstraints.noSelfGeneralization(),
			MetaModelDiagramEdgeConstraints.noSelfDependency(),
			MetaModelDiagramEdgeConstraints.noDirectCycles(DependencyEdge.class),
			MetaModelDiagramEdgeConstraints.noDirectCycles(GeneralizationEdge.class),
			MetaModelDiagramEdgeConstraints.noDirectCycles(AggregationEdge.class),
			MetaModelDiagramEdgeConstraints.noDirectCycles(AssociationEdge.class),
			MetaModelDiagramEdgeConstraints.noCombinedAssociationAggregation()
	);
	 ...
	 */
	public MetaModelDiagramBuilder( Diagram pDiagram )
	{
		super( pDiagram );
		assert pDiagram.getType() == DiagramType.METAMODEL;
	}
	...
	{
		assert pNodes != null;
		return pNodes.stream().allMatch(MetaModelDiagramBuilder::isLinkable);
	}
	...
	/*
	 * Returns true if all the nodes in pNodes are detachable
	 * @pre pNodes != null && pNodes.size() > 0
	 */
	private static boolean allUnlinkable(List<Node> pNodes)
	{
		assert pNodes != null && pNodes.size() > 0;
		return pNodes.stream().allMatch(MetaModelDiagramBuilder::isUnlinkable);
	}
